...
var room_scene = preload("res://tileset/Room.tscn")
var player = load("res://players/p1.tscn")
...
onready var room_container = get_parent().get_parent().get_node("Rooms")

var has_room_generated = false
...
func generate_dungeon():
	...
	if not is_debug:
		make_map()
	add_player()
	emit_signal("room_generated")
	has_room_generated = true
...
func add_player():
	...
	var sr = r.pop_front()
	p = player.instance()
	get_tree().get_root().get_node("test").get_node("YSort").add_child(p)
	p.position = sr.get_center() - sr.size/2
	Global.Player = p
...
func find_mst(positions):
	...
	while positions:
		...
		var closest_point = astar_map.get_closest_point(p)
		astar_map.connect_points(closest_point, n)
		positions.erase(min_p)
		#random loop added
		for i in range(randi() % 2):
			astar_map.connect_points(randi()%n,n)
	return astar_map
...
func make_map():
	...
	var topleft = world_to_map(full_rect.position)
	var bottomright = world_to_map(full_rect.end)
	...
	for room in room_container.get_children():
		...
		for x in range(space_between_rooms, s.x * 2 - (space_between_rooms - 1)):
			for y in range(space_between_rooms, s.y * 2 - (space_between_rooms - 1)):
				set_cell(x + ul.x, y + ul.y, 2, false, false, false, Vector2(3, 3))
...
func carve_path(pos1, pos2):
	...
	for x in range(pos1.x, pos2.x, x_diff):
		...
		for w in (corridor_width - 1):
			set_cell(x, x_y.y + (y_diff * w), 2, false, false, false, Vector2(3, 3))
	for y in range(pos1.y, pos2.y, y_diff):
		if randomize_corridor_width_every_step:
			corridor_width = floor(rand_range(min_corridor_width, max_corridor_width))
		for w in (corridor_width - 1):
			set_cell(y_x.x + (x_diff * w), y, 2, false, false, false, Vector2(3, 3))
